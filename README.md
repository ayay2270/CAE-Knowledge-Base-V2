...
打開網站就可以直接瀏覽。右上角「登入管理」才進入管理；登入後才能新增、編輯、刪除知識。登出後立刻回到可瀏覽的知識庫。

軟體分類存在 Supabase，所有瀏覽器看到同一份。未登入只能瀏覽和篩選。登入後可以在左側「軟體」旁重新命名、新增，或從導覽移除分類。移除只是隱藏（`is_active = false`），不會刪掉既有知識，也不會改寫它們的分類代碼。若分類清單還沒建立，請再執行一次 `setup.sql`。
...
- **換了一台電腦**：直接打開網址即可瀏覽；要編輯再登入。
- **備份**：登入後，右上角人像選單 → 匯出備份（JSON）。
